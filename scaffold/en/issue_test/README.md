...
Every issue must have a corresponding independent regression script placed in the `.agent-workflow/issue_test/` directory.

## Naming Convention

- File name: `.agent-workflow/issue_test/<issue_id>.sh`
- `issue_id` must match `.agent-workflow/docs/plan/current.md` and `.agent-workflow/docs/plan/archive/<issue_id>.md`
...
6. Historical `.agent-workflow/issue_test/*.sh` are retained permanently by default. All subsequent issues must pass all of them. Hiding regressions by deleting, skipping, or weakening old scripts is forbidden.
7. If a historical issue test must be modified, the reason must be recorded in `.agent-workflow/docs/plan/current.md`. If the modification changes the long-term verification semantics, it must also be appended to `.agent-workflow/docs/decisions.md`.
