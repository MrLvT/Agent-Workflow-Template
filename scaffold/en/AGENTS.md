...
11. Stage 4 is responsible for creating a reproducible local delivery state and archiving it; Stage 6 is responsible for final documentation and state alignment. The workflow ends at local delivery and state closeout. If a team still wants additional publication or synchronization steps, humans should handle them outside this workflow.
12. `.agent-workflow/docs/run_log.md` must be maintained continuously: Stage 2 clarifies the goal, Stage 4/6 append work and results, and any stopping point must fill in the end time and final status. `run_log.md` is for run- and Stage-level process tracking; it must not be replaced by `results/.../SUMMARY.md`.
...
14. Only issues that actually execute result-producing experiments, evaluations, benchmarks, or exploratory smoke tests must maintain `results/issue<issue_id>/` and `results/issue<issue_id>/SUMMARY.md`. Once such runs occur, successful, failed, and inconclusive runs must all be logged with setup, model/workflow, input length, result, and attempted analysis. `SUMMARY.md` is for experiment outcome and analysis only, not Stage/process recap, and it must maintain an issue-level synthesis describing what the accumulated evidence says about the issue's core question.
15. After one issue loop has closed, `stage.lock` has returned to `stage1/done/previous=stage6`, and the working tree is clean, remaining on the previous issue branch is not a blocker. Stage 2 may derive the next issue branch directly from the current HEAD. Only unrelated branches or a dirty working tree count as branch blockers.
