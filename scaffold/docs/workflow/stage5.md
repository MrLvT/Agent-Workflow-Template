...
创建 `.agent-workflow/docs/plan/archive/REFLECT-<meta.issue_id>.md`，必须包含以下三个必答项：
...
如果 Step 1 中判断有新的可复用成功模式，追加到 `.agent-workflow/docs/wisdom.md`：
...
如果 Step 1 中判断有新的失败模式，追加到 `.agent-workflow/docs/antipatterns.md`：
...
- 重要设计决策 → 追加到 `.agent-workflow/docs/decisions.md`
- 架构边界变化 → 更新 `.agent-workflow/docs/architecture.md`
- 规范变化 → 更新 `.agent-workflow/docs/conventions.md`
...
- [ ] `.agent-workflow/docs/plan/archive/REFLECT-<meta.issue_id>.md` 已创建（必须存在）
